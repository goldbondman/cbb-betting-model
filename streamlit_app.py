...
# Optional persistence: model_configs table
# Columns recommended:
# id (text), name (text), config_json (jsonb), is_active (bool), created_at (timestamptz)

def sb_save_model_config(cfg: dict, is_active=False):
    payload = {
        "id": cfg.get("meta", {}).get("version_id", f"cfg-{int(datetime.now().timestamp())}"),
    ...
    _, err = sb_try("model_configs", "save model config", _do)
    return err

def sb_load_model_configs():
    def _do(tbl):
        return tbl.select("*").order("created_at", desc=True).limit(50).execute()
    resp, err = sb_try("model_configs", "load model configs", _do)
...
def sb_set_active_model_config(version_id: str):
    def _deactivate(tbl):
        return tbl.update({"is_active": False}).neq("id", "___nope___").execute()
    def _activate(tbl):
        return tbl.update({"is_active": True}).eq("id", version_id).execute()

    _, err1 = sb_try("model_configs", "deactivate all configs", _deactivate)
    _, err2 = sb_try("model_configs", "activate selected config", _activate)
    return err1 or err2
...
# ============================================================
    ...
    with c1:
        if st.button("Save as new version"):
            ...
            if err:
                ...
                st.success("Saved to Supabase model_configs.")
    with c2:
    ...
    if err:
        st.info("Model configs in Supabase not available. If you want persistence, create a table named model_configs.")
    else:
        if configs:
            options = [f"{c.get('name')} | {c.get('id')} | active={c.get('is_active')}" for c in configs]
            pick = st.selectbox("Load a saved model config", options=options, index=0)
            if st.button("Load selected"):
                selected = configs[options.index(pick)]
                cfg_loaded = selected.get("config_json")
                if isinstance(cfg_loaded, dict):
                    set_active_model_config(cfg_loaded)
                    st.success("Loaded into app.")
                else:
                    st.error("Selected row has no config_json.")
