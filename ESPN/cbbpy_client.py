...
def fetch_scoreboard_cbbpy(date_yyyymmdd: str) -> Optional[Dict[str, Any]]:
    ...
    CBBpy's get_game_ids() only returns game IDs, not the full event structure
    (competitions, competitors, scores, status) that parse_scoreboard_event()
    requires. Always returns None so the caller falls back to the direct ESPN
    API, which provides the complete scoreboard payload.
    ...
    """
    logger.info(
        "CBBpy scoreboard skipped for %s: CBBpy get_game_ids() cannot provide "
        "the full event structure (competitions/competitors) required by the "
        "pipeline. Falling back to direct ESPN API.",
        date_yyyymmdd,
    )
    return None
...
def fetch_summary_with_cbbpy_fallback(event_id: str, timeout: int = 25) -> Dict[str, Any]:
    ...
    # circular calls through espn_http_client.fetch_summary).
...
def fetch_scoreboard_with_cbbpy_fallback(date_yyyymmdd: str, timeout: int = 25) -> Dict[str, Any]:
    ...
    # Fall back to direct ESPN API (call fetch_with_retry directly to avoid
    # circular calls through espn_http_client.fetch_scoreboard).
